...
l <- mclapply(1:nrow(param), function(i)
{
	...
	out <- try({
		sim <- tryx.simulate(param$nid[i], param$nu1[i], param$nu2[i], param$bu3y[i], param$bxu3[i], param$bxy[i], outliers_known=param$outliers_known[i], directional_bias = param$directional_bias[i]) %>% tryx.sig()
		res <- sim %>% 
			tryx.analyse()
		res$mvres <- sim$mvres
		res
...
save(l, param, file=paste0("../results/scratch/sim7_", chunk, ".rdata"))
