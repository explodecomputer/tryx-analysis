...
```{r}
x <- sumu3 %>% group_by(ngxu3, mediation)
a <- x %>% nest %>% slice(1) %>% unnest
a %>% arrange(desc(bias)) %>% mutate(rank=1:n())

overall <- bind_rows(
	sumu1 %>% filter(!grepl("Hybrid", method)) %>%
	group_by(prop_invalid_bin) %>% 
	arrange(desc(bias)) %>%
	...
	group_by(prop_invalid_bin, directional_bias2) %>% 
	arrange(desc(bias)) %>%
	mutate(rank=1:n(), what="bias") %>%
	select(method, set, rank, what),
...
grid.arrange(
	...
	p3+th1 + ylim(c(0,1)), 
	p5+th1 + ylim(c(0,1)), 
	p2+th2 + ylim(c(0,1)) + facet_wrap(~ ""), 
	p4+th1 + ylim(c(0,1)), 
	p6+th1 + ylim(c(0,1)) + theme(legend.position=c(0.75,0.4), legend.background=element_blank(), legend.text=element_text(size=6)) + labs(colour=""),
